...
export const getDivespot = createAsyncThunk('divespot/getDivespot', async (divespotId) => {
  const response = await axios.get(`/admin/review/${divespotId}`);
...
export const saveDivespot = createAsyncThunk(
  ...
  async (divespotData, { dispatch, getState }) => {
    ...
    var response;
  if(id){
     response = await axios.patch(`/admin/review/${id}`, divespotData);
  }else{
    response = await axios.post(`/admin/review`, divespotData);
  }
